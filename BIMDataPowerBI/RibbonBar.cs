...
namespace BIMDataPowerBI
{
  public class RibbonBar
  {
    ...
    private void addRibbonBar(UIControlledApplication application)
    {
      ...
      AddPushButtonWithImage(panel, mypath, "button1", " Text ", "BIMDataPowerBI.Command1", "icon-dialog_32x32.png");
      AddPushButtonWithImage(panel, mypath, "button2", " Web Request ", "BIMDataPowerBI.Command2", "icon-web_32x32.png");

      // Create a panel with 1 image button
      RibbonPanel imagePanel = application.CreateRibbonPanel(tabName, "Model Data");
      AddPushButtonWithImage(imagePanel, mypath, "button3", "collect", "BIMDataPowerBI.Command3", "icon-get_32x32.png");
      AddPushButtonWithImage(imagePanel, mypath, "button4", "send", "BIMDataPowerBI.Command4", "icon-send_32x32.png");
    }
    ...
    private void AddPushButtonWithImage(RibbonPanel panel, string dllPath, string name, string text, string className, string imagePath)
    {
      ...
      pushButton.LargeImage = new BitmapImage(new Uri(imagePath, UriKind.Relative));
    }
  }
}
